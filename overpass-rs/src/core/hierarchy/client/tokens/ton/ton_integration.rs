// ./src/core/tokens/ton/ton_integration.rs
use codec::Encode;
...
/// Core TON integration struct
pub struct TON<T: TONConfig> {
    phantom: PhantomData<T>,
}

impl<T: TONConfig> TON<T>
where
    T::NativeCurrency: Currency<T::AccountId>,
{
    /// Create a new TON instance
    pub fn new() -> Self {
        Self {
            phantom: PhantomData,
        }
    ...
    /// Deposit TON to an account
    pub fn deposit(
        &self,
        network: TokenNetwork,
        who: &T::AccountId,
        amount: T::Balance,
    ) -> DispatchResult where <<T as TONConfig>::NativeCurrency as Currency<<T as TONConfig>::AccountId>>::Balance: From<<T as TONConfig>::Balance> {
        match network {
            TokenNetwork::TON => {
    ...
    where 
        <T as TONConfig>::Balance: From<<<T as TONConfig>::NativeCurrency as Currency<<T as TONConfig>::AccountId>>::Balance>,
        <<T as TONConfig>::NativeCurrency as Currency<<T as TONConfig>::AccountId>>::Balance: From<<T as TONConfig>::Balance>
    {
        match network {
            TokenNetwork::TON => {
    ...
    pub fn can_slash(
        ...
        who: &T::AccountId,
        amount: T::Balance,
    ) -> bool where <<T as TONConfig>::NativeCurrency as Currency<<T as TONConfig>::AccountId>>::Balance: From<<T as TONConfig>::Balance> {
        match network {
            TokenNetwork::TON => {
    ...
    where
        T::Balance: From<<<T as TONConfig>::NativeCurrency as Currency<<T as TONConfig>::AccountId>>::Balance>
    {
        match network {
            TokenNetwork::TON => {
