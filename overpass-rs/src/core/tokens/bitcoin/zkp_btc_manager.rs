...
impl ZkProofManager {
    ...
    pub fn create_proof_slice(
        ...
        let proof_data = amount.into_serde().map_err(|e| e.to_string())?;
        ...
        let boc = ZkProofBoc {
            proof_data,
            vk_hash: self.calculate_hash(&proof_data).try_into().map_err(|_| "Invalid hash length")?, // Generate actual verification key hash
        ...
        let boc_hash = self.calculate_hash(&boc_data).try_into().map_err(|_| "Invalid hash length")?;
